...
Square.prototype.isInSameAny = function isInSameAll(square) {
  return this.isInSameColumn(square) || this.isInSameRow(square) || this.isInSameDiag(square);
};

Square.prototype.getDirectionOf = function inDir(square) {
...
Square.prototype.isClearPathTo = function isClearPathTo (destSquare) {
  var curSquare = this;
  var dir = this.getDirectionOf(destSquare); 
  
  while(curSquare !== destSquare) {
    curSquare = curSquare.getAdj(dir);
    if (!curSquare.isEmpty) return;
  }
  return true;
}
